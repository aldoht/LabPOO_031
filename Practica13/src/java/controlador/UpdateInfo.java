...
public class UpdateInfo extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try {
                user.setIdUser(Integer.parseInt(request.getParameter("idUp")));
            }
            catch (NumberFormatException e) {
                out.println("<h3>ERROR: " + e.toString() + "</h3>");
            }
            user.setFname(request.getParameter("fnameUp"));
            user.setLname(request.getParameter("lnameUp"));
            out.println("<h3>" + user.updateData(conn) + "</h3>");
            out.println("</body>");
            out.println("</html>");
        }
    }
